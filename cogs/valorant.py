...
class ValorantCog(commands.Cog, name='Valorant'):
    ...
    @app_commands.command(description='라이엇 계정으로 봇에 로그인합니다')
    @app_commands.describe(username='아이디를 입력하세요', password='비밀번호를 입력하세요')
    ...
    @app_commands.command(description='로그아웃을 하고 계정을 DB에서 완전히 삭제합니다')
    # @dynamic_cooldown(cooldown_5s)
    ...
    @app_commands.command(description="계정의 오늘의 상점을 보여줍니다")
    @app_commands.describe(username='아이디를 입력하세요 (로그인 제외)', password='비밀번호를 입력하세요 (로그인 제외)')
    ...
    @app_commands.command(description='발로란트 포인트 ( VP ) 와 레디어나이트 포인트 ( RP )를 확인시켜줍니다')
    @app_commands.guild_only()
    # @dynamic_cooldown(cooldown_5s)
    ...
    @app_commands.command(description='일일 / 주간 미션을 확인시켜줍니다')
    # @dynamic_cooldown(cooldown_5s)
    async def misson(self, interaction: Interaction, username: str = None, password: str = None) -> None:
    ...
    @app_commands.command(description='야시장에 뜬 스킨들을 보여줍니다')
    # @dynamic_cooldown(cooldown_5s)
    ...
    @app_commands.command(description='현재 배틀패스 티어를 보여줍니다')
    # @dynamic_cooldown(cooldown_5s)
    ...
    @app_commands.command(description="특정한 번들의 정보를 줍니다")
    @app_commands.describe(bundle="알아보고 싶은 특정한 번들의 이름!")
    @app_commands.guild_only()
    # @dynamic_cooldown(cooldown_5s)
    ...
    # inspired by https://github.com/giorgi-o
    @app_commands.command(description="현재의 번들을 알려줍니다")
    # @dynamic_cooldown(cooldown_5s)
    ...
    async def cookies(self, interaction: Interaction, cookie: str) -> None:
        ...
        view = ui.View()
        view.add_item(ui.Button(label="튜토리얼", emoji="🔗", url="https://youtu.be/cFMNHEHEp2A"))
    ...
    @app_commands.command(description='봇의 명령어 디버그')
    @app_commands.describe(bug="고치고 싶은 버그")
    @app_commands.guild_only()
    @owner_only()
    async def debug(
